		...
		<form id="newuser" action="./newuser" method="post">
			...
			<table style="width: 90%;margin:0 auto;">
				...
				<tbody>
					...
					<tr>
						<td><label class="required" for="username">이름</label></td>
						<td><input class="col2"  type="text" id="username" name="username" placeholder="이름" /></td>
					</tr>
					<tr>
						<td><label class="required" for="phone">휴대전화</label></td>
						<td><input class="col2"  type="tel" id="phone" name="phone" placeholder="010-1234-5678" pattern="(010)-\d{3,4}-\d{4}" title="전화번호" /></td>
					</tr>
					<tr>
...
$(function() {
	$('#btn-regist').click(function(event) {
		...
		console.log("${CPATH}/user/checkuser");
		$.post("${CPATH}/user/checknewuser", formdata).done(function(data) {
			console.log(data);
			if (data.error) {
