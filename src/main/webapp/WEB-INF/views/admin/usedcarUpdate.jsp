...
//		}	
		...
		for (var nArrCnt in optionList) {
			if(optionList[nArrCnt] != "")
             	$("input[name=options][value="+optionList[nArrCnt]+"]").prop("checked",true);
		}
		var iconList = "${usedCarVO.icon }".split('|');
		for(var nCnt in iconList){
			if(iconList[nCnt] != "")
				$("input[name=icon][value="+iconList[nCnt]+"]").prop("checked", true);
		}
		$("#maker").val("${usedCarVO.maker }").prop("selected",true);
		$("#status").val("${usedCarVO.sell_state }").prop("selected",true);
		$("#car_type").val("${usedCarVO.car_type }").prop("selected",true);
		$("#fuel").val("${usedCarVO.fuel }").prop("selected",true);
		$("#vehicle_year").val("${usedCarVO.vehicle_year }".substr(0,4)).prop("selected",true);
		$("#vehicle_mon").val("${usedCarVO.vehicle_year }".substr(4,2)).prop("selected",true);

		var arrPeriod = ["24","36","48"];
		if(arrPeriod.indexOf("${usedCarVO.period }") >= 0){
			$("#period").val("${usedCarVO.period}").prop("selected",true);
		}else{
			$("#period").val("0").prop("selected",true);
			document.getElementById("in_period").style.display = "block";
			$("#in_period").val("${usedCarVO.period}");
		}
...
//        $($('.panel-body img')[0]).addClass('summernoteImage');
		...
		formData.append("id","${usedCarVO.id }");
...
<div class="cont-area">    
 <form id="usedCarVO" action="usedcarAction">
<!-- 에디터 관련 -->
        ...
        <ul class="info_list">
          <li class="info_item">
            ...
            <div class="input_wrap">
              <input type="text" placeholder="입력창(텍스트)" name="car_no" id="car_no" value="${usedCarVO.car_no }">
            </div>
          </li>
          <li class="info_item">
          ...
          <li class="info_item">
            ...
            <div class="input_wrap">
              <input type="text" placeholder="입력창(텍스트)" name="name" id="name" value="${usedCarVO.trim_name }">
...
<c:forEach var="month" begin="01" end="12" step="1">
	<option value="${month<10 ? '0':''}${month}">${month}월식</option>
</c:forEach>
              </select>
            </div>
          </li>
          ...
          <li class="info_item">
            ...
            <div class="input_wrap">
              <input class="box_style" type="number" name="deposit"  id="deposit" value="${usedCarVO.deposit }">
              <span class="suffix">
                원
              </span>
            </div>
          ...
          <li class="info_item">
            <p class="info_title ">월렌트료(표준)</p>
            <div class="input_wrap price_wrap">
              <div class="price_item">
                <input class="box_style" type="number" name="rentfee" id="rentfee" value="${usedCarVO.rentfee }">
                <span class="suffix">
                  원
                </span>
              </div>
              <div class="price_item sub_price_item">
                <span class="prefix">24개월렌트료</span>
                <input class="box_style" type="number" name="rentfee_24" id="rentfee_24" value="${usedCarVO.rentfee_24 }">
                <span class="suffix">
                  원
                </span>
              </div>
              <div class="price_item sub_price_item">
                <span class="prefix">한달렌트료</span>
                <input class="box_style" type="number" name="rentfee_1" id="rentfee_1" value="${usedCarVO.rentfee_1 }">
                <span class="suffix">
                  원
                </span>
              </div>
          ...
          <li class="info_item">
            ...
            <div class="input_wrap ">
              <input class="box_style" type="number" name="trim_price" id="trim_price" value="${usedCarVO.trim_price }">
              <span class="suffix">
                원
              </span>
            </div>
          ...
          <li class="info_item">
            ...
            <div class="input_wrap ">
              <input class="box_style" type="number" name="acquisition" id="acquisition" value="${usedCarVO.acquisition }">
              <span class="suffix">
                원
              </span>
            </div>
          ...
          <li class="info_item">
            ...
            <div class="input_wrap">
              <input type="number" placeholder="1부터" name="rank" id="rank" value="${usedCarVO.ranking }">
            </div>
          </li>
          <li class="info_item">
          ...
          <li class="info_item">
            ...
            <div class="input_wrap">
              <input class="text_box" type="number" placeholder="Km(숫자)" name="distance" id="distance" value="${usedCarVO.distance }">
              <span class="suffix">
                km
              </span>
            </div>
      ...
      <div class="right_area">
        ...
        <div class="text_section">
          ...
          <div class="textarea_wrap">
            <textarea name="etc_memo"  id="etc_memo">${usedCarVO.etc_memo }</textarea>
          </div>
        </div>
        ...
        <div class="text_section">
          ...
          <div class="textarea_wrap">
            <textarea name="accident" id="accident">${usedCarVO.accident }</textarea>
          </div>
        </div>
      </div>
    </div>
    <div class="bottom_area" >
		<div style="height:300px; border:1px solid #ccc" id="summernote" class="summernote"></div>
		<textarea name="detailDesc" id="detailDesc" style="display: none;"  rows="10" cols="10">${usedCarVO.contents }</textarea>
