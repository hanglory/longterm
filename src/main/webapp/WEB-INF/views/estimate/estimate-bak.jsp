			...
			<div class="detail">
				...
				<table id="select-item">
					...
					<tr>
						...
						<td id="model-name" class="ta-center">
							${model }
						</td>
					</tr>
					...
					<tr>
						...
						<td id="color-name" class="ta-right">${color }</td>
					</tr>
					<tr>
						<td><span class="title-header">◆</span> 차량 가격</td>
						<td id="car-price" class="ta-right">
							<fmt:formatNumber type="number" maxFractionDigits="3" value="${trimprice}" /> 원
						</td>
					</tr>
					<tr>
						<td><span class="title-header">◆</span> 옵션 가격</td>
						<td id="option-price" class="ta-right">
							<fmt:formatNumber type="number" maxFractionDigits="3" value="${optionprice}" /> 원
						</td>
					</tr>
					<tr>
						<td><span class="title-header">◆</span> 총 가격</td>
						<td id="total-price" class="ta-right">
							<fmt:formatNumber type="number" maxFractionDigits="3" value="${trimprice + optionprice}" /> 원
						</td>
					</tr>
					<tr>
						<td><span class="title-header">◆</span> 계약기간</td>
						<td id="period" class="ta-right"> ${period }개월</td>
					</tr>
					<tr>
						<td><span class="title-header">◆</span> 주행거리</td>
						<td id="distance" class="ta-right">
							<fmt:formatNumber type="number" maxFractionDigits="3" value="${distance / 10000}" />만Km
						</td>
					</tr>
				</table>
			</div>
			<div class="detail">
				...
				<div class="brief">
					<table id="result">
						<tr>
							<td colspan="2">
								<img id="car-image" src="${RPATH }/images/car/${image}" alt="" />
							</td>
						</tr>
						<tr>
							<td>선납금</td>
						...
						<tr>
							<td>보증금</td>
							<td id="deposit">
								<fmt:formatNumber type="number" maxFractionDigits="3" value="${deposit / 10000}" /> 만원<br>
								(차량가의 <fmt:formatNumber type="number" maxFractionDigits="3" value="${depositratio *100}" />%)
							</td>
						</tr>
						<tr>
							<td>월렌트료</td>
							<td id="rentfee">
								<fmt:formatNumber type="number" maxFractionDigits="3" value="${rentfee}" /> 원
							</td>
						</tr>
					</table>
					<div id="screen-box">
						...
						<span>수수료 : <fmt:formatNumber type="number" maxFractionDigits="3" value="${agent_fee}" /> 원</span>
						</c:if>
					</div>
				</div>
		...
		<div class="pdf_page common-item">	
				...
				<table>
					...
					<tr>
						...
						<td id="acquisition">
							<fmt:formatNumber type="number" maxFractionDigits="3" value="${acquisition/10000 }" /> 만원
						</td>
					</tr>
				</table>
...
document.getElementById('btn-save').addEventListener('click', function () {
	...
	xhttp.onreadystatechange = (e)=>{
		...
		if(req.readyState === XMLHttpRequest.DONE) {
			...
			if(req.status === 200) {
				...
				document.getElementById("estimate-no").innerHTML = data.estimate_no;
	...
	else if (("${MEMBERS}" != 0) && (estimate.user_id.length == 0)) {
