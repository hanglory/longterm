...
//			car.acquisition_hi = data.acquisition;
			car.jangrate_hi = parseInt(data.jang_rate)+15;
			car.acquisition_hi = ((car.trim_price + car.total_option_price + car.color_price) * (car.jangrate_hi))/100;
			//car.acquisition_hi = (data.cal_price * ((parseInt(data.jang_rate)+15) /100 ));
			...
			add_rentfee = fAdd_rentfee(1, data.cal_price);
			rentfeeMonth = Math.ceil( (Number(data.rentfee) + Number(add_rentfee)) * 1.1 /1000) * 1000;
			car.rentfee_hi = rentfeeMonth;
...
//			car.rentfee_hi = Math.round( car.rentfee_hi - (car.preprice_hi*10000/car.period_hi));
			...
			$('#acquisition_hi').html(Number(Math.round( car.acquisition_hi / 10000)).toLocaleString('en') + "만원");
...
// 마이렌트
	...
	$.ajax({	
		...
		success: function(data) {
			...
			//car.acquisition_my = (data.cal_price * ( (parseInt(data.jang_rate)+4) / 100 ) );
			car.jangrate_my = parseInt(data.jang_rate)+4;
			car.acquisition_my = ((car.trim_price + car.total_option_price + car.color_price) * (car.jangrate_my))/100;
			car.agentfee_my = data.agent_fee;
			car.cal_price = data.cal_price;

			add_rentfee = fAdd_rentfee(2,data.cal_price);
			rentfeeMonth = Math.ceil( (Number(data.rentfee) + Number(add_rentfee)) * 1.1 /1000) * 1000;
			car.rentfee_my = rentfeeMonth;
...
//			car.rentfee_my = Math.round( car.rentfee_my - (car.preprice_my*10000/car.period_my));
			$('#acquisition_my').html(Number(Math.round( car.acquisition_my / 10000)).toLocaleString('en') + "만원");
			$('#deposit_my').html(Number(Math.round( car.deposit_my / 10000)).toLocaleString('en') + "만원");
			$('#rentfee_my').html(Number(car.rentfee_my).toLocaleString('en') + "원/월");
...
//			car.jangrate_ou = data.jang_rate;
//			car.agentfee_ou = data.agent_fee;
 ...
 //무심사
	...
	$.ajax({   
		...
		success: function(data) {
			...
			//car.acquisition_no = (data.cal_price * ((parseInt(data.jang_rate)+15) /100 ));
			car.jangrate_no = parseInt(data.jang_rate)+15;
			car.acquisition_no = ((car.trim_price + car.total_option_price + car.color_price) * (car.jangrate_no))/100;
			car.agent_fee_rate = car.agent_fee_rate_no;
			var no_price = Math.floor((car.preprice_no*10000/car.period_no)/1000)*1000;

			add_rentfee = fAdd_rentfee(4,data.cal_price);
			rentfeeMonth = Math.ceil( (Number(data.rentfee) + Number(add_rentfee) ) * 1.1 /1000) * 1000;
			car.rentfee_no = rentfeeMonth;			
//			$('#rentfee_no').html(Number( car.rentfee_no - no_price).toLocaleString('en') + "원/월");
			//car.rentfee_no = Number( car.rentfee_no - no_price);
			$('#acquisition_no').html(Number(Math.round( car.acquisition_no / 10000)).toLocaleString('en') + "만원");
			$('#deposit_no').html(Number(Math.round( car.deposit_no / 10000)).toLocaleString('en') + "만원");
...
var fAdd_rentfee = function(kind , org_price) {
	add_rentfee = org_price;
	if(kind == 1){		//하이렌트
		add_rentfee = Math.round( org_price * 0.004913 + 94097);		
	}else if(kind==2){		//마이렌트
		add_rentfee = Math.round( org_price * 0.000625 + 94097);
	}else if(kind == 4){		//동반차(무심사)
		add_rentfee = Math.round( org_price * 0.006413 + 94097);		
	}
	return add_rentfee;
}
