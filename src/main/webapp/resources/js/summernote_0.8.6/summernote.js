...
(function(factory) {
    ...
    var dom = (function() {
        ...
        var isVideoIframe = function(node) {
			if("DIV" == node.nodeName.toUpperCase() 
					&& "IFRAME-STYLE NOTE-VIDEO-CLIP" == node.className.toUpperCase() 
					&& node.childElementCount == 1
					&& "BR" == node.childNodes[0].nodeName.toUpperCase()) {
				return true;
			}
    ...
    var Context = function($note, options) {
        ...
        /*
            this.rpanInvokeHandler = function () {
            	
                var linkInfo = this.invoke('editor.getLinkInfo');
                var range = this.invoke('editor.saveRange');
                var context = this;
        //        context.invoke('editor.createRange');
            	...
            	var $editor = this.layoutInfo.editor;
            	var $editable = this.layoutInfo.editable;
            	var options = this.options;
            	var lang = options.langInfo;
            	
            	var isChecked = linkInfo.isNewWindow !== undefined ? linkInfo.isNewWindow : options.linkTargetBlank;
            	var body = '' +
        		    '	<div class="pop-inner">'+
        		    ...
        		    '							<input type="text" id="textToDisplay" name="textToDisplay" value="' + linkInfo.text + '"/>' +
        		    '							<button type="button" class="delete">삭제</button>' +
        		    '						</span>' +
        		    '					<p class="tit-h2"><label for="linkUrl">URL 삽입</label></p>' +
        		  	'						<span class="input-style">' +
        		  	'							<input type="text" id="linkUrl" name="linkUrl" value="'+linkInfo.url+'" placeholder="유효한 URL을 입력하세요"/>' +
        		  	'							<button type="button" class="delete">삭제</button>' +
        		  	'						</span>'
        		  	if(isChecked){
        		  		body +=	'					<br><br><input type="checkbox" checked="checked" id="newWinCheck"><label for="newWinCheck">새창에서 열기</label>'
        		  	}else{
        		  		body +=	'					<br><br><input type="checkbox" id="newWinCheck"><label for="newWinCheck">새창에서 열기</label>'
        		  	}
        	    	body +=	'					</div> ' +
        			'					<div class="pop-btn">' +
        		 ...
        		 $('button.bodrb').on('click', function(){
         		 ...
         		 $('button.red').on('click', function(){
         			 ...
         			 var restore = $('#summernote').summernote('restoreRange');
        ...
        this.insertParagraph = function(editable) {
            ...
            if (splitRoot) {
                ...
                if (dom.isEmpty(splitRoot) && dom.isLi(splitRoot)) {
                    ...
                    // 009 isVideoIframe 추가
                    if (
                    		( (dom.isHeading(nextPara) || dom.isPre(nextPara) || dom.isCustomStyleTag(nextPara) ) && dom.isEmpty(nextPara) )
                    		|| dom.isVideoIframe(nextPara)
                    		) {
    ...
    var Editor = function(context) {
        ...
        this.ajaxFileUpload = function (files) {
        	  var fileObj;
        	  var validCheck = true;
    	    	...
    	    	var formData = new FormData();
    	    	  ...
    	    	  formData.append("group2Id", $('#group2Id').val());
    	    		...
    	    		for (var i = 0, l = files.length; i < l; i++) {
    	    			formData.append('uploadImageFiles', files[i]);
    	    		}
        ...
        this.isOverSize = function() {
//        	$editable.html(checkValid($editable.html()));
        	checkValid();
        	return checkBytes($editable.html());
        };
    ...
    var Dropzone = function(context) {
        ...
        this.initialize = function() {
            if (options.disableDragAndDrop) {
            	...
            	this.attachDragAndDropEvent();	//주석: 20170906 막아두면 섬머노트 드래그앤 드랍 안됨, base64파일첨부 안됨, 다른 기능은 잘됨./
            }
        };
        ...
        this.attachDragAndDropEvent = function() {
            ...
            $dropzone.on('drop', function(event) {
                ...
                if (dataTransfer && dataTransfer.files && dataTransfer.files.length) {
                    ...
                    context.invoke('editor.ajaxFileUpload', dataTransfer.files);
        ...
        this.addLinkPopoverButtons = function() {
            context.memo('button.linkDialogShow', function() {
                return ui.button({
                    ...
                    //			rpan 전용 popover창으로 변경, 기본으로 하고싶을때는 해당 click이벤트를 사용한다.          
                    //          click: context.createInvokeHandler('linkDialog.show')
                    click: function() {
                        context.rpanInvokeHandler();
                    }
...
*/
    ...
    function checkValid(){
    	//제거 대상
    	$('div.note-editable.panel-body').find('meta, script').remove();
//    	return str.replace('<meta http-equiv="Content-Type" content="text/html;charset=UTF-8">', '');
    }
	...
	function checkBytes(str){
	    ...
	    for (var k = 0 ; k < len ; k++){
	        ...
	        if(count > max) {
	            alert("작성 내용(이미지 등을 포함한)은 10MB까지만 가능합니다.\n\rHTML로 변환하는 과정에서 이미지는 많은 용량을 차지하는 경우가 있습니다.");
	            return true;
	        }
	    }
